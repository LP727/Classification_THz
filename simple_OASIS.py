...
for i in range(1):
    for j in range(1):
        for k in range (1):
            ...
            model.add(tf.keras.layers.Conv2D(layer1, (3, 3), activation='relu', input_shape=(IMG_X_SIZE, IMG_Y_SIZE, 3)))
            model.add(tf.keras.layers.MaxPooling2D((2, 2)))
            model.add(tf.keras.layers.Dropout(0.25))
            model.add(tf.keras.layers.Conv2D(layer2, (2, 2), activation='relu'))
            model.add(tf.keras.layers.MaxPooling2D((2, 2)))
            model.add(tf.keras.layers.Dropout(0.25))
            ...
            model.compile(optimizer=tf.keras.optimizers.RMSprop(
                          ...
                          metrics=['accuracy'])
            model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint('./data/output/ckp/OASIS.hdf5', save_best_only=True, monitor='val_accuracy', mode='max')
            # train model head.
