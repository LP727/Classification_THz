...
basic_model = tf.keras.models.load_model('./data/output/ckp/HCP_rep.hdf5')
rl_model = tf.keras.models.load_model('./data/output/ckp/HCP.hdf5')
...
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(basic_fpr, basic_tpr, label='HCP model without dropout(area = {:.3f})'.format(basic_auc))
plt.plot(rl_fpr, rl_tpr, label='HCP model with dropout(area = {:.3f})'.format(rl_auc))
